...
const formatFlowLabel = (value: string) => {
  const overrides: Record<string, string> = {
    ...
    nvidia: 'Nvidia Mentions',
    salesforce: 'Salesforce Mentions',
    databricks: 'Databricks Mentions',
    ibm: 'IBM Mentions',
...
const buildReportClassificationBreakdown = (
  ...
  const branches: ReportClassificationBreakdownBranch[] = [
    ...
    },
    {
      ...
      children: vendorTags.map(tag => ({
        id: tag,
        label: tag,
        count: countBySetValue(vendorReports, report => report.vendorTags, tag),
        pctOfParent: toPct(countBySetValue(vendorReports, report => report.vendorTags, tag), vendorCount),
      })),
    },
    {
      id: 'harm',
