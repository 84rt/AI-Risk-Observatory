...
Paths: legacy `output/...` locations are now under `/data` (`data/raw`, `data/processed`, `data/results`).
...
[2. PREPROCESSING] → PreprocessedReport (Markdown)
    ↓
[3. STORAGE] → Saved to data/processed/preprocessed/
    ↓
...
save_to_file()
    ↓
data/processed/preprocessed/{strategy}/{company}.md
```
...
**Location**: `data/processed/preprocessed/risk_only/*.md`
...
**Input**: Markdown file from `data/processed/preprocessed/{strategy}/`
...
markdown = read_file("data/processed/preprocessed/risk_only/00077536_REL.md")
...
|------|----------|---------|
| **Input** | `data/raw/ixbrl/*.xhtml` | `549300WSX3VBUFFJOO66_RELX_PLC_2024-12-31.xhtml` |
| **Preprocessed** | `data/processed/preprocessed/{strategy}/*.md` | `00077536_REL.md` |
| **Test Samples** | `data/results/samples/{strategy}/*.md` | `RELX_risk_only.md` |
...
- [x] Files saved to data/processed/preprocessed/
...
1. Read markdown files from `data/processed/preprocessed/`
2. Chunk the text
