...
def filter_chunks_for_classifier(
    ...
    Args:
        offset: If > 0, skip the first `offset` matching chunks.
...
def run_phase2(
    ...
    filtered = filter_chunks_for_classifier(
        chunks,
        classifier_name,
        limit=limit,
        offset=offset,
...
def get_diff_indices(results: list[dict]) -> list[int]:
    """Return row indices where human and LLM labels differ."""
    return [
        i for i, r in enumerate(results)
        if compare_sets(r["human_labels"], r["llm_labels"]) != "EXACT"
    ]
...
def show_details(
    ...
    """Show detailed view for selected rows (or disagreement rows)."""
    print("\n" + "=" * 80)
    print("DETAILED VIEW")
    print("=" * 80)

    selected_indices = set(indices) if indices is not None else None
    if diff_only and selected_indices is None:
        selected_indices = set(get_diff_indices(results))

    for i, r in enumerate(results):
        if selected_indices is not None and i not in selected_indices:
            continue
        match = compare_sets(r["human_labels"], r["llm_labels"])
        if selected_indices is None and match == "EXACT":
            continue
...
filtered_chunks = filter_chunks_for_classifier(
    chunks,
    CLASSIFIER_NAME,
    limit=LIMIT,
    offset=OFFSET,
